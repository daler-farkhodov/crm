...
export function AddStudentModal({ classes }: { classes: ClassOption[] }) {
  const tt = useT();
        ...
        >
          <div className="relative w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-xl dark:border-slate-600 dark:bg-slate-900">
            <button
              ...
              aria-label={tt("students.modal.close")}
            >
              <X className="h-5 w-5" />
            </button>
            <h2
            ...
            >
              {tt("students.modal.title")}
            </h2>
            <p className="mb-6 text-sm text-slate-500 dark:text-slate-400">
              {tt("students.modal.subtitle")}
            </p>
